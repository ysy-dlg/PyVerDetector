...
$(BUILD_DIR)/$(TARGET): $(BUILD_DIR)/main.c scanner.c $(BUILD_DIR)/versions.h $(SCANNERS)
	echo "[CC] $@"
...
$(BUILD_DIR)/2.3.l: $(BUILD_DIR)/2.4.l
	echo "[GEN] $@"
	mkdir -p $(dir $@)
	egrep -v '^"@"' $< | sed -e 's/py24/py23/' | sed -e 's/2.4.tab.h/2.3.tab.h/' > $@
...
$(BUILD_DIR)/3.5.l: $(ORIG_SCANNER_V3)
	echo "[GEN] $@"
	mkdir -p $(dir $@)
	sed -e '/^{name}/i"@="       { return ATEQ; }\n"await"    { return AWAIT; }\n"async"    { return ASYNC; }' $< | sed -e 's/py33/py35/' | sed -e 's/3.3.tab.h/3.5.tab.h/' > $@
